...
def embed_product_chunk(handle, sku, text):
    ...
    product â€” deletes any existing embedded document for this handle
    first, same re-run-safe pattern as the bulk sync script."""
    conn = get_db_conn()
    if not conn:
        print("[PHASE1] embed_product_chunk skipped â€” no DB connection")
        return
    try:
        with conn.cursor() as cur:
            cur.execute(
...
def build_order_status_reply(orders):
    ...
    if not orders:
        return ("I couldn't find a recent order linked to this number. "
                "Could you share your order number so I can check for you? ðŸ˜Š")
    lines = ["Here's what I found for your recent order(s):\n"]
    for o in orders:
        status = o["fulfillment_status"].replace("_", " ").title()
        lines.append(
            f"ðŸ§¾ Order #{o['order_number']}\n"
            f"ðŸ’° Total: Rs.{o['total_price']}\n"
            f"ðŸ“¦ Status: {status}\n"
        )
    lines.append("Let me know if you need anything else! ðŸ˜Š")
    return "\n".join(lines)
...
# â”€â”€ Customer Intent / Lead Intelligence â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Ordered highest-signal to lowest â€” first match wins, so "how to order"
...
def classify_intent(message):
    """Rule-based message-level intent classification â€” same simple-rules
...
def ai_reply():
    ...
    # â”€â”€ 4a-2. Order status â€” look up real Shopify order, no Groq â”€â”€
